...
bool AudioEngine::iniciar() {
    ...
    if (ma_engine_init(nullptr, &engine_) != MA_SUCCESS) {
        std::cerr << "[audio] Nao foi possivel abrir o dispositivo de audio.\n"
                  << "[audio] A mesa vai rodar em modo silencioso (as batidas "
...
AudioEngine::~AudioEngine() { finalizar(); }
